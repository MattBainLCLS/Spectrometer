...
def find_Avantes_devices():

    avaspec.AVS_Init(0)
    found_device_list = list()
    for device in avaspec.AVS_GetList():
        found_device_list.append(FoundDevice(manufacturer = "Avantes", model = device.UserFriendlyName.decode("UTF-8"), serial = device.SerialNumber.decode("UTF-8")))
...
class Avantes():
    ...
    def grab(self):
        ...
        avs_callback = avaspec.AVS_MeasureCallbackFunc(self.measure_complete_callback)
    ...
    def measure_complete_callback(self, pparam1, pparam2):
